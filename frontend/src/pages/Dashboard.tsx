  ...
  const loadData = async () => {
    try {
      ...
      const [appointmentsRes, doctorsRes] = await Promise.all([
        ...
        user?.role === 'patient' ? authAPI.listDoctors() : Promise.resolve({ data: [] }),
      ]);
          ...
          <div className="flex items-center space-x-2 sm:space-x-4">
            ...
            <div className="hidden sm:flex items-center pl-2 ml-2 space-x-3 border-l border-gray-200">
              ...
              <div className={`w-10 h-10 rounded-full flex items-center justify-center text-sm font-bold text-white shadow-md ${
                user?.role === 'doctor' ? 'bg-gradient-to-r from-emerald-400 to-teal-500' : 'bg-gradient-to-r from-blue-400 to-indigo-500'
              }`}>
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8 space-y-8">
        ...
        {nextAppointment && (
          <div className="transform transition-all active:scale-[0.99] duration-200 animate-in fade-in slide-in-from-bottom-5 delay-75">
            <div className={`rounded-2xl shadow-lg border overflow-hidden relative ${
              user?.role === 'doctor' 
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                        <div className="flex items-start space-x-4">
                          <div className={`w-12 h-12 rounded-full flex items-center justify-center flex-shrink-0 shadow-sm ${
                            user?.role === 'doctor' ? 'bg-indigo-100 text-indigo-600' : 'bg-emerald-100 text-emerald-600'
                          }`}>
